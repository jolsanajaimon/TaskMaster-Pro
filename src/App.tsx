          ...
          <div className="p-8 bg-slate-50/50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800">
            <form onSubmit={addTask} className="space-y-4">
              <div className="relative group">
                <input type="text" value={inputValue} onChange={e => setInputValue(e.target.value)} placeholder="Add a new task..." className="w-full pl-6 pr-14 py-5 bg-white dark:bg-slate-900 border-2 border-slate-200 dark:border-slate-700 focus:outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 transition-all text-lg text-slate-700 dark:text-slate-200 placeholder:text-slate-400 font-medium shadow-sm" />
                <button type="submit" disabled={!inputValue.trim()} className="absolute right-3 top-3 bottom-3 px-5 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 disabled:opacity-50 transition-all shadow-lg shadow-indigo-200 dark:shadow-none flex items-center justify-center active:scale-95">
              ...
              <div className="flex flex-wrap gap-3">
                <div className="flex items-center gap-2 bg-white dark:bg-slate-900 px-4 py-2 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
                  ...
                  <select value={priority} onChange={e => setPriority(e.target.value as any)} className="text-sm font-semibold text-slate-600 dark:text-slate-400 outline-none bg-transparent cursor-pointer">
                ...
                <div className="flex items-center gap-2 bg-white dark:bg-slate-900 px-4 py-2 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
                  ...
                  <select value={category} onChange={e => setCategory(e.target.value)} className="text-sm font-semibold text-slate-600 dark:text-slate-400 outline-none bg-transparent cursor-pointer">
                ...
                {filteredTasks.length > 0 ? filteredTasks.map(t => {
                  ...
                  return (
                    <motion.li key={t.id} layout initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: -40 }}
                      ...
                      <button onClick={() => toggleTask(t.id)} className={`flex-shrink-0 transition-all transform active:scale-90 ${t.completed ? "text-emerald-500" : "text-slate-300 dark:text-slate-600 group-hover:text-indigo-400"}`}>
                        {t.completed ? <CheckCircle2 className="w-8 h-8" /> : <Circle className="w-8 h-8" />}
                      </button>
                  ...
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="py-20 text-center">
                    ...
                    <p className="text-slate-400 dark:text-slate-500 font-medium">{searchQuery ? "Try a different search term" : "Start by adding your first task above"}</p>
                  </motion.div>
                )}
              </AnimatePresence>
            </ul>
